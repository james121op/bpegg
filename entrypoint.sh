...
function Update() {
    ...
    # backup files
    cp -r "{Maps,www,Plugins,server_info.txt,announcements.txt,groups.json,settings.json,videos.json,skins.txt,whitelist.txt,npc_names.txt}" /home/container/tmp/filesafe
    tar xzf bp.tar.gz
    rm -rf bp.tar.gz
    #put files back (not plugins they need to be updated)
    cp -r "/home/container/tmp/filesafe/{Maps,www,server_info.txt,announcements.txt,groups.json,settings.json,videos.json,skins.txt,whitelist.txt,npc_names.txt}" /home/container
    Done
    exit 0
}
